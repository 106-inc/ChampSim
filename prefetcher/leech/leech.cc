...
public:
  ...
  uint32_t tag[ST_SET][ST_WAY][NUM_CPUS], last_offset[ST_SET][ST_WAY][NUM_CPUS], sig[ST_SET][ST_WAY][NUM_CPUS], lru[ST_SET][ST_WAY][NUM_CPUS];
...
public:
  ...
  uint32_t c_delta[PT_SET][PT_WAY][NUM_CPUS], c_sig[PT_SET][NUM_CPUS];
  ...
  void update_pattern(uint32_t last_sig, int curr_delta, int cpu),
      read_pattern(uint32_t curr_sig, int* prefetch_delta, uint32_t* confidence_q, uint32_t& lookahead_way, uint32_t& lookahead_conf, uint32_t& pf_q_tail,
                   uint32_t& depth, uint32_t pq_occupancy, uint32_t pq_SIZE, uint32_t mshr_occupancy, uint32_t mshr_SIZE, int cpu);
};
...
public:
  ...
  uint64_t pf_useful[NUM_CPUS], pf_issued[NUM_CPUS],
  ...
  uint32_t sig[MAX_GHR_ENTRY][NUM_CPUS], confidence[MAX_GHR_ENTRY][NUM_CPUS], offset[MAX_GHR_ENTRY][NUM_CPUS];
...
void CACHE::prefetcher_initialize() { std::cout << "LEECH PREFETCH" << std::endl; }
...
uint32_t CACHE::prefetcher_cache_operate(uint64_t addr, uint64_t ip, uint8_t cache_hit, uint8_t type, uint32_t metadata_in)
{
  ...
  if (NUM_CPUS == 4) { // Multi-core config
    ...
    uint32_t page_offset = (addr >> LOG2_BLOCK_SIZE) & (PAGE_SIZE / BLOCK_SIZE - 1), last_sig = 0, curr_sig = 0, confidence_q[LLC_MSHR_SIZE], depth = 0;

    int32_t delta = 0, delta_q[LLC_MSHR_SIZE];
    ...
    if (last_sig)
      PT_L.update_pattern(last_sig, delta, cpu);

    uint32_t lookahead_conf = 100;
    // Stage 3: Start prefetching

    uint64_t base_addr = addr;
    uint32_t pf_q_head = 0, pf_q_tail = 0;
    ...
    do {
      ...
      PT_L.read_pattern(curr_sig, delta_q, confidence_q, lookahead_way, lookahead_conf, pf_q_tail, depth, PQ.occupancy(), PQ.size(), MSHR.size(), LLC_MSHR_SIZE,
                        cpu);
            ...
            if (num_pf < ((MSHR.size()) / distinct_pages) + 1) {
              if (FILTER_L.check(pf_addr, SPP_LLC_PREFETCH, cpu)) {
                ...
                if (confidence_q[i] >= PF_THRESHOLD_LLC) {
                  ...
                  SPP_DP(cout << "[ChampSim] SPP L2 prefetch issued GHR_L.pf_issued: " << GHR_L.pf_issued[cpu] << " GHR_L.pf_useful: " << GHR_L.pf_useful[cpu]
                              << endl;);
                }
      ...
      if (lookahead_way < PT_WAY) {
        ...
        int sig_delta = (PT_L.delta[set][lookahead_way][cpu] < 0) ? (((-1) * PT_L.delta[set][lookahead_way][cpu]) + (1 << (SIG_DELTA_BIT - 1)))
                                                                  : PT_L.delta[set][lookahead_way][cpu];
...
{
  uint32_t set = get_hash_l(page) % ST_SET, match = ST_WAY, partial_page = page & ST_TAG_MASK;
  uint8_t ST_hit = 0;
  int sig_delta = 0;
    ...
    if (valid[set][match][cpu] && (tag[set][match][cpu] == partial_page)) {
      ...
      if (delta) {
        ...
        SPP_DP(cout << "[ST] " << __func__ << " hit set: " << set << " way: " << match;
               ...
               cout << " last_sig: " << last_sig << " curr_sig: " << curr_sig; cout << " delta: " << dec << delta << " last_offset: " << page_offset << endl;);
      } else
...
void PATTERN_TABLE_L::update_pattern(uint32_t last_sig, int curr_delta, int cpu)
{
  ...
  uint32_t set = get_hash_l(last_sig) % PT_SET, match = 0;
...
void PATTERN_TABLE_L::read_pattern(uint32_t curr_sig, int* delta_q, uint32_t* confidence_q, uint32_t& lookahead_way, uint32_t& lookahead_conf,
                                   uint32_t& pf_q_tail, uint32_t& depth, uint32_t pq_occupancy, uint32_t pq_SIZE, uint32_t mshr_occupancy, uint32_t mshr_SIZE,
                                   int cpu)
{
  // Update (sig, delta) correlation
  uint32_t set = get_hash_l(curr_sig) % PT_SET, local_conf = 0, pf_conf = 0, max_conf = 0;
  ...
  if (c_sig[set][cpu]) {
    for (uint32_t way = 0; way < PT_WAY; way++) {
      ...
      // debug
      if (pf_conf > 100) {
        cout << "[PT] " << __func__ << " CONF ERROR!! c_delta: " << c_delta[set][way][cpu] << " c_sig: " << c_sig[set][cpu]
             << " Alpha: " << GHR_L.global_accuracy[cpu] << endl;
      }
    ...
    if (lookahead_conf >= PF_THRESHOLD_LLC)
      depth++;
...
{
  uint64_t cache_line = check_addr >> LOG2_BLOCK_SIZE, hash = get_hash_l(cache_line), quotient = (hash >> REMAINDER_BIT) & ((1 << QUOTIENT_BIT) - 1),
  ...
  case SPP_LLC_PREFETCH:
    if ((valid[quotient][cpu] || useful[quotient][cpu]) && remainder_tag[quotient][cpu] == remainder) {
      ...
      SPP_DP(cout << "[FILTER] " << __func__ << " set valid for check_addr: " << hex << check_addr << " cache_line: " << cache_line << dec;
             cout << " quotient: " << quotient << " remainder_tag: " << remainder_tag[quotient][cpu] << " valid: " << valid[quotient][cpu]
                  << " useful: " << useful[quotient][cpu] << endl;);
    }
    break;
  ...
  case LLC_DEMAND:
    if ((remainder_tag[quotient][cpu] == remainder) && (useful[quotient][cpu] == 0)) {
      ...
      if (valid[quotient][cpu])
        GHR_L.pf_useful[cpu]++; // This cache line was prefetched by SPP and actually used in the program
  ...
  case LLC_EVICT:
    ...
    if (valid[quotient][cpu] && !useful[quotient][cpu] && GHR_L.pf_useful[cpu])
      GHR_L.pf_useful[cpu]--;
...
void GLOBAL_REGISTER_L::update_entry(uint32_t pf_sig, uint32_t pf_confidence, uint32_t pf_offset, int pf_delta, int cpu)
{
  ...
  SPP_DP(cout << "[GHR] Replace index: " << victim_way << " pf_sig: " << hex << sig[victim_way] << dec;
         cout << " confidence: " << confidence[victim_way][cpu] << " pf_offset: " << offset[victim_way][cpu] << " pf_delta: " << delta[victim_way][cpu]
              << endl;);
