...
{
    std::cout << "LEECH PREFETCH" << std::endl;
}
...
if (NUM_CPUS == 4) { // Multi-core config
    ...
    do {
        ...
        PT_L.read_pattern(curr_sig, delta_q, confidence_q, lookahead_way, lookahead_conf, pf_q_tail, depth, PQ.occupancy(), PQ.size(), MSHR.size(), LLC_MSHR_SIZE, cpu);
